...
//!
//! Window functions (`SELECT`s containing `ROW_NUMBER`/`RANK`/`LAG`/etc.)
//! bypass the VM the same way `JOIN` does: codegen reconstructs the parsed
//! `Query` as a literal Rust value and calls
//! [`crate::query::execute_windowed`] with it at runtime -- no `const
//! PROGRAM`, since `execute_windowed` partitions/sorts entirely outside the
//! register-machine model.

use crate::query::{self, output_column_names, AggPart};
use crate::sql::{
    self, AggFunc, BinOp, Expr, Join, JoinKind, Literal, OrderBy, ParseError, Query, SelectItem,
...
pub fn generate(sql_text: &str) -> Result<String> {
    ...
    {
        return Ok(render_windowed(sql_text, &query));
    }
...
fn render(
    ...
    let _ = writeln!(
        out,
        "//! Generated by column-rs codegen v{version} -- DO NOT EDIT"
    );
    ...
    out.push_str(
        "        let rows = column_rs::query::run_program(&file, COLUMNS_TO_LOAD, PROGRAM)?;\n",
    );
...
fn render_multi_table(sql_text: &str, query: &Query, other_table: &str, exec_fn: &str) -> String {
    ...
    let _ = writeln!(
        out,
        "    let rows = column_rs::query::{exec_fn}(&main_file, &other_file, &query)?;"
    );
    out.push_str("    println!(\"{}\", COLUMNS.join(\"\\t\"));\n");
    out.push_str("    for row in rows {\n");
...
/// [`crate::query::execute_windowed`].
fn render_windowed(sql_text: &str, query: &Query) -> String {
    let columns = output_column_names(query);
    let mut out = String::new();
    let version = env!("CARGO_PKG_VERSION");
    let _ = writeln!(
    ...
    out.push_str("#![allow(unused_imports)]\n");
    out.push_str("use column_rs::file::ParquetFile;\n");
    out.push_str("use column_rs::sql::{AggFunc, BinOp, Expr, OrderBy, Query, SelectItem, WindowFunc, WindowSpec};\n\n");
    ...
    out.push_str("        let file = ParquetFile::open(&data)?;\n");
    out.push_str("        let query = build_query();\n");
    out.push_str("        let rows = column_rs::query::execute_windowed(&file, &query)?;\n");
    out.push_str("        for row in rows {\n");
    out.push_str(
        "            let line: Vec<String> = row.iter().map(|v| v.to_string()).collect();\n",
    );
...
mod tests {
    ...
    fn generates_query_literal_for_semi_join() {
        let src = generate("SELECT id FROM orders WHERE region_key IN (SELECT key FROM regions)")
            .unwrap();
        assert!(
    ...
    fn rejects_more_than_one_join() {
        let err =
            generate("SELECT a.id FROM a JOIN b ON a.id = b.id JOIN c ON a.id = c.id").unwrap_err();
        assert!(matches!(
            err,
            CodegenError::Unsupported("more than one JOIN")
        ));
    }
